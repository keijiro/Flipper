...
namespace Flipbook
{
    ...
    public sealed class FlipbookRenderer : MonoBehaviour
    {
        ...
        [SerializeField] bool _holdWhileEditing = true;
        ...
        class Page
        {
            ...
            {
                RT = RenderTexture.GetTemporary(resolution.x, resolution.y, 24);
        ...
        float DeltaTime {
            get {
                return (Application.isPlaying || !_holdWhileEditing) ?
                     Time.deltaTime : 10;
            }
        }
        ...
        Page RenderPage()
        {
            var page = (_freePages.Count > 0) ?
                _freePages.Dequeue() : new Page(_resolution);
        ...
        void LateUpdate()
        {
            ...
            if (_time > _interval)
            {
                // Render a new page and calculate the next interval.
                _activePages.Enqueue(RenderPage());
                _time %= _interval;
            }

            // Add delta to the active pages and the last one.
            var delta = DeltaTime * _speed / _interval;
